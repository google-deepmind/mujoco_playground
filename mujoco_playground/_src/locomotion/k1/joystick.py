...
class Joystick(k1_base.K1Env):
  ...
  def _post_init(self) -> None:
    ...
    # Per-joint action scale, following booster_train's convention:
    # scale = 0.25 * effort_limit / kp. Effort limits live on the joints
    # (actuatorfrcrange), not on the position actuators.
    kp = self._mj_model.actuator_gainprm[:, 0]
    actuated_joints = self._mj_model.actuator_trnid[:, 0]
    effort_limit = self._mj_model.jnt_actfrcrange[actuated_joints, 1]
    self._action_scale = jp.array(
        0.25 * effort_limit / kp
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    rng, key = jax.random.split(rng)
    qpos = qpos.at[7:].set(
        qpos[7:] * jax.random.uniform(key, (22,), minval=0.5, maxval=1.5)
    )
  ...
  def _cost_feet_distance(
    ...
    return jp.clip(0.18 - feet_distance, min=0.0, max=0.1)
