...
class BraxDomainRandomizationVmapWrapper(Wrapper):
    ...
    # swapping added to solve the UnexpectedTracerError as described in https://github.com/google/brax/issues/609
    @contextmanager
    def _swap_sys(self, new_sys):
        env = self.env.unwrapped
        old_sys = env._mjx_model
        env._mjx_model = new_sys
        try:
            yield
        finally:
            env._mjx_model = old_sys
