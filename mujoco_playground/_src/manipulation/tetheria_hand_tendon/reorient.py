...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        # history_len=1,  # original mujoco playground training
        history_len=5,
        obs_noise=config_dict.create(
            level=1.0,
            scales=config_dict.create(
                joint_pos=0.05,
                # cube_pos=0.02, # original mujoco playground training
                cube_pos=0.05,
                cube_ori=0.1,
            ),
...
class CubeReorient(tetheria_hand_base.TetheriaHandEnv):
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        data = mjx_env.init(
            self.mjx_model,
            qpos=qpos,
            ctrl=q_hand[self._control_qids_bool],
            qvel=qvel,
        ...
        info = {
            ...
            "qpos_error_history": jp.zeros(self._config.history_len * 15),
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        motor_targets = jp.clip(
            motor_targets,
            self._lowers[self._control_qids_bool],
            self._uppers[self._control_qids_bool],
        )
        motor_targets = (
    ...
    def _get_obs(self, data: mjx.Data, info: dict[str, Any]) -> mjx_env.Observation:
        ...
        qpos_error_history = (
            jp.roll(info["qpos_error_history"], 15)
            .at[:15]
            .set(noisy_joint_angles[self._control_qids_bool] - info["motor_targets"])
        )
        ...
        state = jp.concatenate(
            [
                ...
                qpos_error_history,  # 15 * history_len
    ...
    def _cost_energy(self, qvel: jax.Array, qfrc_actuator: jax.Array) -> jax.Array:
        return jp.sum(jp.abs(qvel[self._control_qids_bool]) * jp.abs(qfrc_actuator))
...
def domain_randomize(model: mjx.Model, rng: jax.Array):
    ...
    control_qids = mjx_env.get_qpos_ids(mj_model, consts.CONTROL_JOINT_NAMES)
    hand_body_names = [
        "palm",
        "right_index_f_link",
