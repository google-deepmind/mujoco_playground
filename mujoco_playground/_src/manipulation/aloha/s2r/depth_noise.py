...
def _bilinear_interpolate(image, y, x):
    ...
    y_clamped = jp.clip(y, 0.0, H - 1.0)
    x_clamped = jp.clip(x, 0.0, W - 1.0)
    ...
    y1 = jp.where(y0 < H - 1, y0 + 1, y0)
    x1 = jp.where(x0 < W - 1, x0 + 1, x0)
    ...
    # Compute the bilinear interpolated result. Need to be careful to avoid dead pixels in image edges.
    return Ia * (1.0 - dx) * (1.0 - dy) + Ib * dx * (1.0 - dy) + Ic * (1.0 - dx) * dy + Id * dx * dy
...
def kinect_noise(key, depth, *, sigma_s=0.5, sigma_d=1/6, baseline=35130):
    ...
    grid_y, grid_x = jp.mgrid[0:H, 0:W].astype(jp.float32)
    
    # Random shifts in x and y (sampled from Gaussian).
    key, key_shift = jax.random.split(key)
    shift_x, shift_y = jax.random.normal(key_shift, (2, H, W)) * sigma_s
    ...
    shifted_depth = _bilinear_interpolate(depth, shifted_y.ravel(), shifted_x.ravel()).reshape(H, W)
    ...
    disparity_noisy = disparity + jax.random.normal(key_noise, (H, W)) * sigma_d
...
def edge_noise(key, depth, *, grad_threshold=0.05, noise_multiplier=10):
    ...
    When the spatial gradient of the depth is greater than threshold, theres a chance the pixels are dropped.
    Then, randomly jitter those dropped pixels.
    Note that the proper way to do this requires the surface normals of everything in the scene.
    ...
    mask = jax.random.uniform(key_dropout, depth.shape) < p_lost # if true, then drop.

    # Scatter the mask.
    H, W = depth.shape
    grid_y, grid_x = jp.mgrid[0:H, 0:W].astype(jp.int32)

    # Random coordinate shifts in x and y, uniformly 0, 1.
    key_shift, key = jax.random.split(key)
    shift_x, shift_y = jax.random.randint(key_shift, (2, H, W), minval=0, maxval=2)
    ...
    shifted_x = jp.clip(shifted_x, 0, W - 1)
    shifted_y = jp.clip(shifted_y, 0, H - 1)
...
def np_get_line_bank(H, W, bank_size=100, color_range=[0, 0.4]):
    """
    Get a bank of random lines. Not jax-compatible.
    Returns a bank of size:
    (bank_size, H, W)
    where each element is a white image with up to max_lines lines randomly drawn on it. 
    """
    max_lines = 16
    bank = []
...
def _or_reduce(arr, axis):
    ...
    That is, it returns the last nonzero element along that axis (or 0 if all are zero).
    ...
    """
    # Flip the array along the specified axis so that the last element comes first.
    ...
    # If all are zero, np.argmax will return 0. (This is fine since arr[...0] is 0.)
