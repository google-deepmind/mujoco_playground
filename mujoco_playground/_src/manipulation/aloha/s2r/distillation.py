...
def default_config() -> config_dict.ConfigDict: # TODO :Clean up. Or just import?
...
def get_frozen_encoder_fn():
    ...
    vision_mlp = networks.VisionMLP(
        layer_sizes = (0,),
        policy_head=False
    )
    from brax.io import model as brax_loader
    fpath = pathlib.Path(__file__).parent / "params" / "VisionMLP2ChanCIFAR10.prms"
...
class DistillPegInsertion(peg_insertion.PegInsertion):
    def __init__(
        ...
        if self._vision:
            ...
            if self._config.obs_noise.depth:
                ...
                self.line_bank = jp.array(depth_noise.np_get_line_bank(
                    H, W, bank_size=16384, color_range=[max_depth * 0.2, max_depth * 0.85]
                ))
    ...
    def reset_color_noise(self, info):
        ...
        def generate_noise(chan):
            ...
            noise = jax.random.uniform(
                ...
                (1, 3),
                minval=0,
                maxval=color_noise_scales[chan]
            )
    ...
    def _get_obs_distill(self, data, info):
        ...
        if not self._vision:
            state_wt = jp.concatenate([obs_insertion, (info['_steps'] / self._config.episode_length).reshape(1)])
        ...
        rgb_l = adjust_brightness(
            self.rgb_noise(rng_noise1, rgb_l, info),
            info['brightness']
        )
        rgb_r = adjust_brightness(
            self.rgb_noise(rng_noise2, rgb_r, info),
            info['brightness']
        )
        latent_rgb_l, latent_rgb_r = jp.split(self.encoder_fn(
            {
                'pixels/view_0': rgb_l,
                'pixels/view_1': rgb_r
            }), 2, axis=-1)
    ...
    def _get_proprio(self, data: mjx.Data, info: Dict) -> jax.Array:
        ...
        qpos_noise = jax.random.uniform(rng, (16,), minval = 0, maxval=self._config.obs_noise.robot_qpos)
    ...
    def process_depth(self, depth, chan: int, view_name: str, key: Optional[jp.ndarray]=None, info: dict=None):
...
def make_teacher_policy():
    ...
    f_pick_teacher = pathlib.Path(__file__).parent / "params" / "AlohaS2RPick.prms"
    f_insert_teacher = pathlib.Path(__file__).parent / "params" / "AlohaS2RPegInsertion.prms"

    teacher_pick_policy = peg_insertion.load_brax_policy(f_pick_teacher.as_posix(), 
                                                         "AlohaS2RPick",
                                                         int(env.action_size/2),
                                                         distill=True)

    teacher_insert_policy = peg_insertion.load_brax_policy(f_insert_teacher.as_posix(), 
    ...
    def teacher_inference_fn(obs, rng):
        ...
        l_act, l_extras = teacher_pick_policy({'state': l_obs}, None) # l_extras: for example, loc: B x act_size.
        ...
        if 'socket_hidden' in obs:
            ...
            r_extras = jax.tree_util.tree_map(
                lambda x: jp.where(obs['peg_hidden'], jp.zeros_like(x), x),
                r_extras
            )
        act_1 = jp.concatenate([l_act, r_act], axis=-1)
        act_extras_1 = jax.tree_util.tree_map(
            lambda x, y: jp.concatenate([x, y], axis=-1),
            l_extras,
            r_extras
        )
...
def domain_randomize(model: mjx.Model, rng: jax.Array):
  ...
  def rand(rng):
    ...
    for cur_idx in cam_ids:
      ...
      cam_offset = jax.random.uniform(rng_pos, (3,), minval=-1, maxval=1) * offset_scales
      cam_pos = cam_pos.at[cur_idx].set(cam_pos[cur_idx] + cam_offset)
      cam_quat = cam_quat.at[cur_idx].set(perturb_orientation(rng_ori, cam_quat[cur_idx], 5))
    ...
    light_offset = jax.random.uniform(
      rng_pos, 
      model.light_pos.shape, 
      minval=-1, maxval=1) * offset_scales
    ...
    return (cam_pos, cam_quat, geom_rgba,
            light_pos, light_dir, light_castshadow)

  (cam_pos, cam_quat, geom_rgba,
   light_pos, light_dir, light_castshadow) = rand(rng)
