...
from mujoco_playground._src.manipulation.aloha import aloha_constants, base
from mujoco_playground._src.mjx_env import State  # pylint: disable=g-importing-member

QPOS_NOISE_MASK_SINGLE = [1]*6 + [0]*2 # 6 joints, 2 fingers.
ZIPF_S3 = [0.83, 0.104, 0.02, 0.014, 0.008, 0.002] # heavy-tailed zipf pmf evaluated at x=1, ..., 6 with s=3.
GRASP_THRESH = 0.015
...
def init_obs_history(init_obs: Dict,
...
def use_obs_history(key, 
                    obs_history: Dict,
                    obs: Dict) -> Tuple[Dict, Dict]:
    """ Purely in-place.
...
class S2RBase(base.AlohaEnv):
    ...
    def _post_init(self, keyframe: str):
        ...
        for obj in self.obj_names:
            setattr(self, f"_{obj}_qposadr", self._mj_model.jnt_qposadr[
                self._mj_model.body(obj).jntadr[0]
            ])
            setattr(self, f"_{obj}_body", self._mj_model.body(obj).id)
            setattr(self, f"_{obj}_init_pos", jp.array(
                self._init_q[getattr(self, f"_{obj}_qposadr") :
                            getattr(self, f"_{obj}_qposadr") + 3],
                dtype=jp.float32,
            ))
            setattr(self, f"_{obj}_grip_site", self._mj_model.site(f"{obj}_grip_here").id)
            setattr(self, f"_{obj}_mocap_target", self._mj_model.body(f"{obj}_mocap_target").mocapid)

        #### PER HAND
        for hand in self.hands:
            setattr(self, f"_{hand}_left_finger_geom_bottom", self._mj_model.geom(f"{hand}/left_finger_bottom").id)
            setattr(self, f"_{hand}_right_finger_geom_bottom", self._mj_model.geom(f"{hand}/right_finger_bottom").id)
            # Fingertips
            setattr(self, f"_{hand}_left_fingertip", self._mj_model.site(f"{hand}/left_fingertip").id)
            setattr(self, f"_{hand}_right_fingertip", self._mj_model.site(f"{hand}/right_fingertip").id)
            setattr(self, f"_{hand}_hand_geom", self._mj_model.geom(f"{hand}/gripper_base").id)
            setattr(self, f"_{hand}_gripper_site", self._mj_model.site(f"{hand}/gripper").id)
            setattr(self, f"_{hand}_base_link", self._mj_model.body(f"{hand}/base_link").id)
            setattr(self, f"_{hand}_qposadr", np.array([
    ...
    def sample_fan(self, rng: jax.Array, obj: str) -> Tuple[jax.Array, jax.Array]:
        ...
        angle = jax.random.uniform(
            rng_angle,
            shape=(),
            minval= -par/2, # Can't be a symmetric fan or depth cameras can't distinguish objects.
            maxval= par/2
        )
        dx = r * jp.cos(angle)
        dy = r * jp.sin(angle)
    ...
    def init_objects(self, rng: jax.Array) -> Tuple[mjx.Data, dict[str, Any]]:
        ...
        for obj, targ, side in zip(self.obj_names, self.target_positions, self.hands): # Defined by child class.
            ...
            base_quat = jp.array([jp.cos(base_angle / 2), 0.0, 0.0, jp.sin(base_angle / 2)])
        ...
        for hand in self.hands:
            ...
            _init_ctrl = jp.array(self._init_ctrl).at[first_idx_ctrl].set(rand_setpoint)
        ...
        for i, obj in enumerate(self.obj_names):
            ...
            data = data.replace(
                mocap_pos=data.mocap_pos.at[mocap_target, :].set(info[f"_{obj}_target_pos"]),
        ...
        info.update({
            ...
            "rng": rng,
            "action_history": jp.zeros((self._config.action_history_length, self.action_size), dtype=jp.float32),
            "motor_targets": _init_ctrl,
            "init_ctrl": _init_ctrl, # TODO: OK for brax PPO resets, breaks brax Dagger resets.
        })
    ...
    def _step(self, state: State, action: jax.Array) -> mjx.Data:
        ...
        state.info['action_history'] = jp.where(
            newly_reset, 
            jp.zeros((self._config.action_history_length, self.action_size), dtype=jp.float32),
            state.info['action_history']
        )

        action_history = jp.roll(state.info["action_history"], 1, axis=0).at[0].set(action)
        ...
        action_idx = jax.random.randint(
            ...
            (),
            minval=0,
            maxval=self._config.action_history_length
        )
    ...
    def _get_obs_pick_helper(self, data: mjx.Data, 
                             info: dict[str, Any], 
                             side: str, obj: str) -> jax.Array:
        """ 
        ...
        g_target_mat = math.quat_to_mat(data.mocap_quat[getattr(self, f"_{obj}_mocap_target")])
        R = data.xmat[getattr(self, f"_{side}_base_link")] # world to local. Orientation.
        t = data.xpos[getattr(self, f"_{side}_base_link")] # world to local. Translation.
        ...
        noise = jax.random.uniform(
            key_qpos, rob_qpos.shape, minval=-self._config.obs_noise.robot_qpos, maxval=self._config.obs_noise.robot_qpos
        ) * jp.array(QPOS_NOISE_MASK_SINGLE)
        n_rob_qpos = rob_qpos + noise
        
        noise = jax.random.uniform(
            key_qvel, rob_qvel.shape, minval=-self._config.obs_noise.robot_qvel, maxval=self._config.obs_noise.robot_qvel
        ) * jp.array(QPOS_NOISE_MASK_SINGLE)
        ...
        n_obj_v = obj_v + jax.random.uniform(
            key_obj_v, obj_v.shape, minval=-self._config.obs_noise.obj_vel, maxval=self._config.obs_noise.obj_vel
        )
        n_obj_angv = obj_angv + jax.random.uniform(
            key_obj_angv, obj_angv.shape, minval=-self._config.obs_noise.obj_angvel, maxval=self._config.obs_noise.obj_angvel
        )
        # GRIPPER, OBJ MAT
        ...
        _noise = jax.random.uniform(
            key_gripper_box, (2, 3), minval=-self._config.obs_noise.gripper_box, maxval=self._config.obs_noise.gripper_box
        )
        n_gripper_box = gripper_box + (_noise[1] - _noise[0]) # Triangle distribution

        # OBJ POS
        info["rng"], key_obj = jax.random.split(info["rng"])
        n_obj_pos = obj_pos + jax.random.uniform(
            key_obj, obj_pos.shape, minval=-self._config.obs_noise.obj_pos, maxval=self._config.obs_noise.obj_pos
        )
