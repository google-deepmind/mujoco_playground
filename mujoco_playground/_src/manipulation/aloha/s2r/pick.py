...
class Pick(base.S2RBase):
  def __init__(
    ...
    self.target_positions = [
        jp.array([-0.1, 0.0, 0.25], dtype=float)
    ]
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    actor_obs, _ = flax.core.pop(obs, 'privileged') # Privileged obs is not randomly shifted.
    info["obs_history"] = base.init_obs_history(actor_obs, self._config.max_obs_delay)
  ...
  def _reset_info_if_needed(self, state: State, newly_reset: bool):
    ...
    state.info['success_time'] = jp.where(newly_reset, 0, state.info['success_time'])

  def _calculate_sparse_rewards(self, state: State, data: mjx.Data) -> Tuple[Dict[str, float], bool, int]:
  ...
  def _calculate_dense_rewards(self, data: mjx.Data, info: Dict[str, Any], metrics: Dict[str, float]) -> Dict[str, float]:
  ...
  def _calculate_reg_rewards(self, data: mjx.Data, info: Dict[str, Any], action: jax.Array, metrics: Dict[str, float]) -> Dict[str, float]:
    ...
    raw_reg_rewards.update({"finger_force": n_f_fing * self.is_grasped(data, "left")})
  ...
  def _check_done_conditions(self, data: mjx.Data, state: State) -> Tuple[bool, bool, bool]:
  ...
  def _update_score(self, state: State, success: bool, done: bool):
    last_step = ((state.info['_steps'] + self._config.action_repeat) >= self._config.episode_length)
    ...
    state.info['_steps'] = jp.where(done | (state.info['_steps'] >= self._config.episode_length), 0, state.info['_steps'])

  def _update_state(self, state: State, data: mjx.Data, total_reward: float, done: bool, newly_reset: bool) -> State:
    ...
    return State(data, obs, total_reward, done.astype(float), state.metrics, state.info)
  ...
  def _calculate_success(self, state: State, box_pos: jax.Array, data: mjx.Data) -> Tuple[bool, int]:
    pos_thresh, rot_thresh = self._calculate_thresholds(state.info['score'])
    success = (jp.linalg.norm(box_pos - state.info['_box_target_pos']) < pos_thresh)
  ...
  def _get_dense_pick(self, data: mjx.Data, info: Dict[str, Any]) -> Dict[str, Any]:
      ...
      info["reached_box"] = 1.0 * jp.maximum(
          info["reached_box"],
          (jp.linalg.norm(self.gripping_error(data, "left", "box")) < base.GRASP_THRESH))
  ...
  def _get_reg_pick(self, data:mjx.Data, info: Dict[str, Any], action: jp.ndarray) -> Dict[str, Any]:
      rewards = {
          "robot_target_qpos": self._robot_target_qpos(data),
      }

      joint_vel_mse = jp.linalg.norm(
          data.qvel[self._left_qposadr]
      )
